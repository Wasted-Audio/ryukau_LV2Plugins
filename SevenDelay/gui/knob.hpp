...
public:
  ...
  {
    auto radian = (2.0 * normalized - 1.0) * (pi - arcNotchHalf);
  ...
  void onNanoDisplay() override
  {
    ...
    beginPath();
    arc(
      centerX, centerY, radius, float(pi) / 2.0f - arcNotchHalf,
      float(pi) / 2.0f + arcNotchHalf, CCW);
    stroke();
    ...
    fillColor(tipColor);
    beginPath();
    arc(tip.getX() + centerX, tip.getY() + centerY, halfArcWidth, -pi, pi, CCW);
    fill();
  }
...
protected:
  ...
  const float arcNotchHalf = float(pi) / 6.0f; // Radian.
