...
class RingDelaySynth : public Plugin {
public:
  RingDelaySynth() : Plugin(ParameterID::ID_ENUM_LENGTH, 0, 0)
  {
...
protected:
  const char *getLabel() const override { return "RingDelaySynth"; }
  const char *getDescription() const override { return "Some delays."; }
  ...
  int64_t getUniqueId() const override { return d_cconst('u', 'r', 'd', 'e'); }
...
private:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(RingDelaySynth)
};

Plugin *createPlugin() { return new RingDelaySynth(); }
