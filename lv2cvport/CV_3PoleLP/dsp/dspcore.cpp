...
void DSPCore::setParameters()
{
  ...
  interpLpDecay.push(
    param.value[ParameterID::dcBlock]->getFloat()
    * LP3<float>::highpassHzToDecay(
      sampleRate, param.value[ParameterID::highpass]->getFloat()));
}
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    lp3.setRaw(
      ...
      std::clamp<float>(
        ...
        sampleRate / 2.0f),
      std::clamp<float>(resonance + inResonance[i], 0.0f, 1.0f),
      std::clamp<float>(decay + 0.1f * inDecay[i], 0.0f, 1.0f), uniformPeak, uniformGain);
    out0[i] = interpGain.process() * lp3.process(in0[i]);
  }
}
