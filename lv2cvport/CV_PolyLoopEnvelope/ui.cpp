...
class CV_PolyLoopEnvelopeUI : public PluginUIBase {
protected:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CV_PolyLoopEnvelopeUI)

public:
  CV_PolyLoopEnvelopeUI() : PluginUIBase(defaultWidth, defaultHeight)
  {
    ...
    addLabel(left0, top0, 2 * knobX, labelHeight, midTextSize, "CV_PolyLoopEnvelope");
    ...
    constexpr size_t nEnvelopeSection = 8;

    const auto leftMatrix0 = left0;
    const std::array<float, nEnvelopeSection> leftMatrix = {
      left0 + 1.0f * knobX, left0 + 2.0f * knobX, left0 + 3.0f * knobX,
      left0 + 4.0f * knobX, left0 + 5.0f * knobX, left0 + 6.0f * knobX,
      left0 + 7.0f * knobX, left0 + 8.0f * knobX,
    };
    const auto leftMatrixRelease = left0 + 9.0f * knobX;
...
UI *createUI() { return new CV_PolyLoopEnvelopeUI(); }
