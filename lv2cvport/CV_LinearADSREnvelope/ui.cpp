...
constexpr uint32_t defaultHeight = uint32_t(labelHeight + 5 * labelY + 40);

enum tabIndex { tabMain, tabPadSynth, tabInfo };

class CV_LinearADSREnvelopeUI : public PluginUI {
protected:
...
private:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CV_LinearADSREnvelopeUI)

public:
  CV_LinearADSREnvelopeUI() : PluginUI(defaultWidth, defaultHeight)
  {
    ...
    addGroupLabel(left0, top0, 2 * knobX, "CV_LinearADSREnvelope");

    const int labelAlign = ALIGN_LEFT | ALIGN_MIDDLE;

    addLabel(left0, top0 + labelY, knobX, "Gain", labelAlign);
    addTextKnob(
      left1, top0 + labelY, knobX, colorBlue, ID::gain, Scales::defaultScale, false, 4);

    addLabel(left0, top0 + 2 * labelY, knobX, "Attack", labelAlign);
    addTextKnob(
      left1, top0 + 2 * labelY, knobX, colorBlue, ID::attack, Scales::attack, false, 4);

    addLabel(left0, top0 + 3 * labelY, knobX, "Decay", labelAlign);
    addTextKnob(
      left1, top0 + 3 * labelY, knobX, colorBlue, ID::decay, Scales::decay, false, 4);

    addLabel(left0, top0 + 4 * labelY, knobX, "Sustain", labelAlign);
    addTextKnob(
      left1, top0 + 4 * labelY, knobX, colorBlue, ID::sustain, Scales::sustain, false, 4);

    addLabel(left0, top0 + 5 * labelY, knobX, "Release", labelAlign);
    addTextKnob(
      left1, top0 + 5 * labelY, knobX, colorBlue, ID::release, Scales::release, false, 4);
  }
};

UI *createUI() { return new CV_LinearADSREnvelopeUI(); }
