...
class CV_ParabolicADEnvelope : public Plugin {
public:
  // Plugin(nParameters, nPrograms, nStates).
  CV_ParabolicADEnvelope() : Plugin(ParameterID::ID_ENUM_LENGTH, 0, 0)
  {
...
protected:
  /* Information */
  const char *getLabel() const override { return "CV_ParabolicADEnvelope"; }
  const char *getDescription() const override { return "Parabolic AD envelope."; }
...
private:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CV_ParabolicADEnvelope)
};

Plugin *createPlugin() { return new CV_ParabolicADEnvelope(); }
