...
class CV_PControllerUI : public PluginUIBase {
protected:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CV_PControllerUI)

public:
  CV_PControllerUI() : PluginUIBase(defaultWidth, defaultHeight)
  {
    ...
    addGroupLabel(left0, top0, 2 * knobX, labelHeight, midTextSize, "CV_PController");

    const int labelAlign = ALIGN_LEFT | ALIGN_MIDDLE;

    addLabel(
      left0, top0 + labelY, knobX, labelHeight, uiTextSize, "Cutoff [Hz]", labelAlign);
    addTextKnob(
      left1, top0 + labelY, knobX, labelHeight, uiTextSize, ID::cutoff, Scales::cutoff,
      false, 3);

    addCheckbox(
      left0, top0 + 2 * labelY, knobX, labelHeight, uiTextSize, "Bypass", ID::bypass);
  }
};

UI *createUI() { return new CV_PControllerUI(); }
