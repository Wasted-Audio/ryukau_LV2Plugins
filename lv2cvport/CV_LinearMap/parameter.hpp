...
struct Scales {
  static SomeDSP::LinearScale<double> fraction;
  static SomeDSP::IntScale<double> integer;
};
...
struct GlobalParameter {
  ...
  GlobalParameter()
  {
    ...
    value[ID::addFrac] = std::make_unique<LinearValue>(
      0.5, Scales::fraction, "addFrac", kParameterIsAutomable);
    value[ID::addInt]
      = std::make_unique<IntValue>(128, Scales::integer, "addInt", kParameterIsAutomable);

    value[ID::mulFrac] = std::make_unique<LinearValue>(
      0.5, Scales::fraction, "mulFrac", kParameterIsAutomable);
    value[ID::mulInt]
      = std::make_unique<IntValue>(129, Scales::integer, "mulInt", kParameterIsAutomable);
  }
