...
class CV_LinearMapUI : public PluginUI {
protected:
...
public:
  CV_LinearMapUI() : PluginUI(defaultWidth, defaultHeight)
  {
    ...
    addGroupLabel(left0, top0, 3 * knobX, "CV_LinearMap");

    const int labelAlign = ALIGN_LEFT | ALIGN_MIDDLE;

    addLabel(left1, top1, knobX, "Integer", labelAlign);
    addLabel(left2, top1, knobX, "Fraction", labelAlign);

    addLabel(left0, top2, knobX, "Mul", labelAlign);
    addTextKnob(
      left1, top2, knobX, colorBlue, ID::mulInt, Scales::integer, false, 0, -128);
    addTextKnob(left2, top2, knobX, colorBlue, ID::mulFrac, Scales::fraction, false, 5);

    addLabel(left0, top3, knobX, "Add", labelAlign);
    addTextKnob(
      left1, top3, knobX, colorBlue, ID::addInt, Scales::integer, false, 0, -128);
    addTextKnob(left2, top3, knobX, colorBlue, ID::addFrac, Scales::fraction, false, 5);

    addLabel(
      left0, top0 + 4 * labelY, 3 * knobX, "Output = Add + Mul * Input.", labelAlign);
  }
};
