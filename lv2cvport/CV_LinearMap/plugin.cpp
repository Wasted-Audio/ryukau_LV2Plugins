...
class CV_LinearMap : public Plugin {
public:
  // Plugin(nParameters, nPrograms, nStates).
  CV_LinearMap() : Plugin(ParameterID::ID_ENUM_LENGTH, 0, 0)
  {
...
protected:
  /* Information */
  const char *getLabel() const override { return "CV_LinearMap"; }
  const char *getDescription() const override
  {
    return "Maps value as: Output = Add + Mul * Input.";
  }
...
private:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CV_LinearMap)
};

Plugin *createPlugin() { return new CV_LinearMap(); }
