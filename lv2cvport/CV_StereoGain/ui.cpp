...
class CV_StereoGainUI : public PluginUI {
protected:
...
public:
  CV_StereoGainUI() : PluginUI(defaultWidth, defaultHeight)
  {
    ...
    addGroupLabel(left0, top0, 2 * knobX, "CV_StereoGain");

    const int labelAlign = ALIGN_LEFT | ALIGN_MIDDLE;

    addLabel(left0, top0 + labelY, knobX, "Gain [dB]", labelAlign);
    addTextKnob(left1, top0 + labelY, knobX, colorBlue, ID::gain, Scales::gain, true, 2);

    addLabel(left0, top0 + 2 * labelY, knobX, "DirectCurrent", labelAlign);
    addTextKnob(left1, top0 + 2 * labelY, knobX, colorBlue, ID::dc, Scales::dc, false, 5);
  }
};
