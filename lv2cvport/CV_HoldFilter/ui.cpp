...
class CV_HoldFilterUI : public PluginUI {
protected:
...
public:
  CV_HoldFilterUI() : PluginUI(defaultWidth, defaultHeight)
  {
    ...
    addGroupLabel(left0, top0, 2 * knobX, "CV_HoldFilter");

    const int labelAlign = ALIGN_LEFT | ALIGN_MIDDLE;

    addLabel(left0, top0 + labelY, knobX, "Gain", labelAlign);
    addTextKnob(left1, top0 + labelY, knobX, colorBlue, ID::gain, Scales::gain, false, 6);

    addLabel(left0, top0 + 2 * labelY, knobX, "Cutoff [Hz]", labelAlign);
    addTextKnob(
      left1, top0 + 2 * labelY, knobX, colorBlue, ID::cutoff, Scales::cutoff, false, 3);

    addLabel(left0, top0 + 3 * labelY, knobX, "Resonance", labelAlign);
    addTextKnob(
      left1, top0 + 3 * labelY, knobX, colorBlue, ID::resonance, Scales::resonance, false,
      6);

    addLabel(left0, top0 + 4 * labelY, knobX, "PulseWidth", labelAlign);
    addTextKnob(
      left1, top0 + 4 * labelY, knobX, colorBlue, ID::pulseWidth, Scales::pulseWidth,
      false, 6);

    addLabel(left0, top0 + 5 * labelY, knobX, "Edge", labelAlign);
    addTextKnob(
      left1, top0 + 5 * labelY, knobX, colorBlue, ID::edge, Scales::edge, false, 6);

    addLabel(left0, top0 + 6 * labelY, knobX, "FilterType", labelAlign);
    std::vector<std::string> filterTypeItems{"Naive", "PolyBLEP 4", "PolyBLEP 6",
                                             "PolyBLEP 8"};
    addOptionMenu(left1, top0 + 6 * labelY, knobX, ID::filterType, filterTypeItems);

    addCheckbox(left0, top0 + 7 * labelY, knobX, "Highpass", ID::highpass);
    addCheckbox(left1, top0 + 7 * labelY, knobX, "OverSampling", ID::overSampling);
  }
};
