...
class CV_PTRSawUI : public PluginUI {
protected:
...
private:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(CV_PTRSawUI)

public:
  CV_PTRSawUI() : PluginUI(defaultWidth, defaultHeight)
  {
    ...
    addGroupLabel(left0, top0, 2 * knobX, "CV_PTRSaw");
    ...
    addLabel(left0, top0 + 3 * labelY, knobX, "Semi", labelAlign);
    addTextKnob(
      left1, top0 + 3 * labelY, knobX, colorBlue, ID::oscSemi, Scales::oscSemi, false, 0,
      -60);

    addLabel(left0, top0 + 4 * labelY, knobX, "Milli", labelAlign);
    addTextKnob(
      left1, top0 + 4 * labelY, knobX, colorBlue, ID::oscMilli, Scales::oscMilli, false,
      0);
  }
};

UI *createUI() { return new CV_PTRSawUI(); }
