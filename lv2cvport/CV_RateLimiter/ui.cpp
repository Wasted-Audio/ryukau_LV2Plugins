...
class CV_RateLimiterUI : public PluginUI {
protected:
...
public:
  CV_RateLimiterUI() : PluginUI(defaultWidth, defaultHeight)
  {
    ...
    addGroupLabel(left0, top0, 2 * knobX, "CV_RateLimiter");

    const int labelAlign = ALIGN_LEFT | ALIGN_MIDDLE;

    addLabel(left0, top0 + labelY, knobX, "Rising Rate", labelAlign);
    addTextKnob(
      left1, top0 + labelY, knobX, colorBlue, ID::risingSlewRate, Scales::slewRate, false,
      5);

    addLabel(left0, top0 + 2 * labelY, knobX, "Falling Rate", labelAlign);
    addTextKnob(
      left1, top0 + 2 * labelY, knobX, colorBlue, ID::fallingSlewRate, Scales::slewRate,
      false, 5);
  }
};
