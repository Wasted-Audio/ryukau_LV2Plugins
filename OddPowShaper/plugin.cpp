...
class OddPowShaper : public Plugin {
public:
...
protected:
  /* Information */
  const char *getLabel() const override { return "OddPowShaper"; }
  const char *getDescription() const override
  {
    return "A waveshaper using following equation: out = sgn(x) * pow(abs(x), (abs(x) % "
           "2 - 1)^2n).";
  }
...
private:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(OddPowShaper)
};

Plugin *createPlugin() { return new OddPowShaper(); }
