...
class WaveCymbal : public Plugin {
public:
  // Plugin(nParameters, nPrograms, nStates).
  WaveCymbal() : Plugin(ParameterID::ID_ENUM_LENGTH, 1, 0)
  {
...
protected:
  ...
  {
    return "A banded-waveguide synthesizer with 1D wave interaction. This can also be "
           "used as FX.";
  }
  ...
  int64_t getUniqueId() const override { return d_cconst('u', 'w', 'c', 'y'); }
...
private:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(WaveCymbal)
};

Plugin *createPlugin() { return new WaveCymbal(); }
