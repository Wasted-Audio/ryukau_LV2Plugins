...
class LightPadSynth : public Plugin {
public:
...
protected:
  /* Information */
  const char *getLabel() const override { return "LightPadSynth"; }
  ...
  {
    if (std::strcmp(key, "padsynth") == 0)
      dsp->refreshTable();
    else if (std::strcmp(key, "lfo") == 0)
      dsp->refreshLfo();
  }
...
private:
  ...
  DISTRHO_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(LightPadSynth)
};

Plugin *createPlugin() { return new LightPadSynth(); }
